...
#ifndef VR_SEURAT_TILER_TANGENTIAL_DISK_COST_FUNCTION_H_
#define VR_SEURAT_TILER_TANGENTIAL_DISK_COST_FUNCTION_H_
...
// Models the tangential distance along a planar disk from its center to the
// projection of points onto that plane.
//
// See Evaluate() for details.
class TangentialDiskCostFunction : public ceres::CostFunction {
 public:
  TangentialDiskCostFunction(absl::Span<const int> points_of_interest,
                             const PointSet& point_set)
      : points_of_interest_(points_of_interest), point_set_(point_set) {}
  ~TangentialDiskCostFunction() override = default;
  ...
  // for the point & normal encoding a plane.
  //
  // A residual is returned for each point-of-interest with
  //   ||center - t_hit * point|| / ||center||
...
#endif  // VR_SEURAT_TILER_TANGENTIAL_DISK_COST_FUNCTION_H_
