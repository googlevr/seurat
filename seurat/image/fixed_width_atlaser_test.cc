...
TEST(FixedWidthAtlaser, GetAtlasSizeTarget) {
  const Vector2i kAtlasSize(10, 20);
  FixedWidthAtlaser atlaser(kAtlasSize);
  EXPECT_EQ(kAtlasSize, atlaser.GetAtlasSizeTarget());
}

TEST(FixedWidthAtlaser, LayoutSingleTile) {
  const Vector2i kAtlasSizeTarget(10, 10);
...
TEST(FixedWidthAtlaser, LayoutTiles) {
  const int kNumRuns = 100;
  const Vector2i kAtlasSizeTarget(64, 64);
