...
#ifndef VR_SEURAT_ARTIFACT_EXR_EXPORTER_H_
#define VR_SEURAT_ARTIFACT_EXR_EXPORTER_H_
...
// Writes Artifacts to disk as .exr files storing their texture.
class ExrExporter : public ArtifactProcessor {
 public:
  explicit ExrExporter(std::shared_ptr<base::FileSystem> filesystem,
                       std::string basename)
      : filesystem_(std::move(filesystem)), basename_(std::move(basename)) {}
  ~ExrExporter() override = default;
...
#endif  // VR_SEURAT_ARTIFACT_EXR_EXPORTER_H_
